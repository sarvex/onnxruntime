...
TEST(MathOpTest, Add_int32) {
  ...
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test    //TensorRT parser: elementwise inputs must not be Int32
}
...
TEST(MathOpTest, Abs_int8) {
  ...
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test    //TensorRT: INT8, Assertion `regionRanges != nullptr' failed
}
...
TEST(MathOpTest, Abs_int32) {
  ...
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test    //TensorRT parser: Int32 not allowed as input to this layer
}
...
#else
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test    //TensorRT: INT8 is not supported
#endif
}
...
TEST(MathOpTest, Neg_int64) {
  ...
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test    //TensorRT parser: Int64 not allowed as input to this layer
}
...
#else
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test                      //TensorRT: Expected output shape [{3,3,3}] did not match run output shape [{3,1,1}] for sum
#endif
}
...
TEST(ModOpTest, Int8_mixed_sign) {
  ...
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {}); // Unleash TRT EP test   // For TensorRT running in these in INT8 quantization scales are needed, so skip it now
}
