...
class RecomputeStart(ComputeNode):
    def __init__(self, reduce_nodes: List[ReduceNode], r_numel: sympy.Expr):
        super().__init__("", [], [])
        self.reduce_nodes = reduce_nodes
        self.r_numel = r_numel


class RecomputeEnd(ComputeNode):
    def __init__(self, reduce_nodes: List[ReduceNode]):
        super().__init__("", [], [])
        self.reduce_nodes = reduce_nodes
...
class DropoutNode(ComputeNode):
    def __init__(self, inputs: List[TensorArg], outputs: List[TensorArg], offset_calc: OffsetCalculator):
        ...
        assert not self.offset_calc.is_single_element(inputs[0].name)
